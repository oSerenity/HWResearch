...
// Created: Ochiai
// Creation date: 120116
// Purpose: Tutorial + War
//
///////////////////////////////////////////////////////////////////////////////////////////////
/*サウンドシーンの付け方 
/ * How to add a sound scene

▼サウンドシーンを作成
▼ Create a sound scene

scr_CreateSceneSound( int id, int se_no, float x, float y, float z );
※id  =サウンドシーンに与えられる識別用の通し番号。自由に設定してください。
※se_no  =SEを呼び出すた目のSE番号。呼び出したいSEの番号を指定してください。
※x、y、ｚ  =音がなるときの音の発生位置の座標です。指定がない場合は０を入れておいてください。

* Id = serial number for identification given to the sound scene. Please set it freely.
* Se_no = SE number of the eye that calls SE. Specify the SE number you want to call.
* X, y, z = Coordinates of the sound generation position when the sound is produced. If not specified, enter 0.

▼ダイアログメッセージの呼び出し 
	scr_PauseOn();	//まずゲームポーズをかけます
	scr_OpenDialog( 0, doc_id );	//doc_idに出すダイアログを指定します。テストとかでしたらT000_01を入れてみてください。 
	while( scr_IsOpenDialog(0) )	//ダイアログが閉じられるまで待つ命令を出します。
	{
		suspend();
	}
	scr_PauseOff();	//ポーズを解除します。
	
▼ Calling a dialog message
scr_PauseOn (); // First, put the game pose
scr_OpenDialog (0, doc_id); // Specify the dialog to be displayed in doc_id. For testing, try inserting T000_01.
while (scr_IsOpenDialog (0)) // Issue a command to wait until the dialog is closed.
{
suspend ();
}
scr_PauseOff (); // Cancel the pause.
*/
...
//	scr_SetDisableTacticalInfomation();	//バナーテキストを非表示 
	...
	//レベルアップ時のスキル習得をやめる 
	scr_PlayerDisableLearningSkillByLevelUp();

	scr_SetMessageCallback( "miscb_PlayerLevelUp", 4222, CB_TYPE_ALWAYS, MESSAGE_PLAYER_LEVEL_UP );
...
gPlayerLV <- 1;
function miscb_PlayerLevelUp()
{
	print("レベルアップした。条件でスキル習得()\n");
	gPlayerLV = (gPlayerLV + 1);

	if(gPlayerLV == 2)
	{
		scr_PlayerLearnSkill( SCR_SPINNING_BLADE, false );
	}
	if(gPlayerLV == 3)
	{
		scr_PlayerLearnSkill( SCR_CHARGE, false );
	}
...
function mis_StartGuardTutorial()
{
	...
	scr_BotPlayerSetMapIconVisible( 200, false );
	suspend();
	mis_Wait( 0.3 );	//待つ 
...
function mis_StartChangeTutorial()
{
	...
	scr_SetMessageCallback( "miscb_InSkillChangeMode", 2907, CB_TYPE_ALWAYS, MESSAGE_HUD_ENABLE_SKILL_SELECT );	//スキルチェンジモードになった 
	scr_SetMessageCallback( "miscb_OutSkillChangeMode", 2807, CB_TYPE_ALWAYS, MESSAGE_HUD_DISABLE_SKILL_SELECT );	//スキルチェンジモードになった 

	scr_SetTimerCallback( "miscb_Lv2", 7654, CB_TYPE_ONCE, 2.0 );
}
gLv2Skillinfo <- 0;
function miscb_Lv2()
{
...
//	mis_MassageDialog( "T000_08b" );	//メッセージ：スキル変えようぜ 
//	mis_Wait( 0.8 );	//レベルアップを待つ 
	mis_MassageDialog( "T000_08c" );	//メッセージ：スキル変えようぜ 

	scr_OpenKeyHelp( SCR_KEY_HELP_SKILL_03, -1.0 );	//キーヘルプ　LB 入力中スキル切り替え可能 
}
gInSkillChange <- 0;
gSkillMode <- 0; //1=inLb 2= outLB 
function miscb_InSkillChangeMode()
{
	print("LBでスキル切り替えモードに()\n");
	scr_CloseKeyHelp();

	if(gLv2Skillinfo == 0)
	{
		scr_RemoveCallbackFromId( 7654 );
		gLv2Skillinfo = 1;
		mis_MassageDialog( "T000_08c" );	//メッセージ：スキル変えようぜ 
	}

	gSkillMode = 1;
	scr_RemoveCallbackFromId( 2007 );
	suspend();
	scr_SetPlayerCurrentSkillCallback( "miscb_ChangeSkill", 2007, CB_TYPE_ONCE, SCR_SPINNING_BLADE );	//初スキル切り替え 

	if(gInSkillChange == 0 )
	{
		scr_RemoveCallbackFromId( 87654 );
		suspend();
		scr_SetTimerCallback( "miscb_InfomationAboutSkillMode", 87654, CB_TYPE_ONCE, 0.4 );
	}
	else
	{
		mis_ChangeKeyHelpSkillMode();
	}
}
function miscb_InfomationAboutSkillMode()
{
//		mis_Wait( 0.4 );
		//初めての場合ダイアログを出す。 
		gInSkillChange = 1;
		mis_MassageDialog( "T000_08d" );	// 	
		mis_ChangeKeyHelpSkillMode();
}
function miscb_OutSkillChangeMode()
{
	print("LBを離してスキル切り替えモードを解除()\n");	
	gSkillMode = 2;
...
function mis_ChangeKeyHelpSkillMode()
{
	if(gSkillMode == 1)
	{
		scr_OpenKeyHelp( SCR_KEY_HELP_COMMAND_02, -1.0 );	//キーヘルプ　LB押しながら A 
	}
	if(gSkillMode == 2)
	{
		scr_OpenKeyHelp( SCR_KEY_HELP_SKILL_03, -1.0 );	//キーヘルプ　LB 入力中スキル切り替え可能 
...
function miscb_ChangeSkill()
{
	print("changeskill()\n");
	scr_CloseKeyHelp();
	scr_RemoveCallbackFromId( 2907 );
	scr_RemoveCallbackFromId( 2807 );
	scr_RemoveCallbackFromId( 87654 );
	if(gInSkillChange == 0)
	{
		gInSkillChange = 1;
	...
	{
		mis_Wait( 0.3 );	//切り替えを待つ 
	}
	mis_Wait( 0.2 );	//切り替えを待つ 
	...
	scr_BotPlayerSetMapIconVisible( 200, true );
	scr_BotPlayerSetMapIconVisible( 300, false );
	suspend();
	mis_Wait( 0.3 );	//待つ 
...
function mis_DeathTrap()
{
	...
	scr_CreateSceneEffect( 10111, "Slis03c", 10947.317383, 130.000000, -1019.666870, 0.0  );	//落雷 
	...
	scr_CreateSceneEffect( 10211, "Slis03c", 10038.220703, 130.000000, -238.202881, 0.0  );	//落雷 

	scr_CreateSceneEffect( 10310, "Slis02c", 10476.724609, 100.000000, -28.096619, 0.0  );	//落雷 
	scr_CreateSceneEffect( 10311, "Slis03c", 10476.724609, 130.000000, -28.096619, 0.0  );	//落雷 

	scr_CreateSceneEffect( 10410, "Slis02c", 10540.187500, 100.000000, -1298.997803, 0.0  );	//落雷 
	scr_CreateSceneEffect( 10411, "Slis03c", 10540.187500, 130.000000, -1298.997803, 0.0  );	//落雷 

	scr_CreateSceneEffect( 10510, "Slis02c", 11182.740234, 100.000000, -566.815186, 0.0  );	//落雷 
	scr_CreateSceneEffect( 10511, "Slis03c", 11182.740234, 130.000000, -566.815186, 0.0  );	//落雷 

	scr_CreateSceneEffect( 10610, "Slis02c", 9892.731445, 100.000000, -735.801331, 0.0  );	//落雷 
	scr_CreateSceneEffect( 10611, "Slis03c", 9892.731445, 130.000000, -735.801331, 0.0  );	//落雷 
...
function mis_StartBattle2()
{
	...
	scr_RegistTrapSphere( 36000, -11845.598633, 100.000000, -2968.014, 220.0 );	//魔法使い攻撃ポジション１（みぎ） 
...
function miscb_StartBallistaEvent()
{
	...
	else
	{
		...
		if(scr_PlayerGetLife() != 0 )
		{
			...
			scr_SetTimerCallback( "miscb_ForceStart", 13, CB_TYPE_ONCE, 10.0 );	//2012/03/12 1５→10に 
		}
		else
		{
...
function miscb_ModeChangeBotEnemyAtMage()
{
	...
	scr_BotPlayerSetRespawnEnable( 210, false );
...
function mis_StartGateEvent()
{
	...
	scr_BotPlayerSetRespawnEnable( 208, true );
	scr_BotPlayerSetRespawnEnable( 209, true );
	scr_BotPlayerSetRespawnEnable( 210, true );
...
function miscb_TrapIn4004AtRam()
{
	...
	scr_BotPlayerLearnSkill( 110, SCR_CONSTRUCT );
	scr_BotPlayerLearnSkill( 111, SCR_CONSTRUCT );
...
//	scr_BotPlayerEnableInterruptActionMode( 115, SCR_ACTION_MODE_DEFENSE, 99999, -1.0 );	//城門を警備 
	}
	else
	{
...
function miscb_CheckRamState()
{
	...
	if( scr_GetGimmickBuildState( "gmon01_002" ) == GIMMICK_BUILD_FREE )	//丸太が資材状態 
	{	
		//解説 
		if(gRamInfo == 0)
		{
...
//		mis_BannerInfoTimer("miscb_BunnerInfoRemove",10.0);	//バナーを削除する 
	...
	{
		if(scr_GetGimmickBuildState( "gmon01_002" ) == GIMMICK_BUILD_COMPLETE)	//丸太が完成状態 
		{
			...
			scr_BotPlayerEnableInterruptActionMode( 115, SCR_ACTION_MODE_DEFENSE, 56000, -1.0 );
...
//			mis_Wait( 2.5 );	//城門崩壊のアニメーションを見守る 
			...
			scr_RemoveCallbackFromId( 11 );
			scr_RemoveCallbackFromId( 12 );
...
function miscb_CastStartCharge()
{
	...
	scr_ResetGameCameraDirection( -16140.387695, -4232.728516, false );
	scr_SetGameCameraAngleType( ANGLE_ZOOM_OUT );	//カメラをズームに設定 
...
function mis_SetRebirthTime()
{
	...
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_BLACK, SCR_REBIRTH_TYPE_PENALTY, 6 );

	scr_SetRebirthTime( SCR_REBIRTH_TARGET_WHITE, SCR_REBIRTH_TYPE_NORMAL, 7 );
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_WHITE, SCR_REBIRTH_TYPE_PENALTY, 7 );
...
function mis_SetRebirthTimeLast()
{
	...
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_BLACK, SCR_REBIRTH_TYPE_NORMAL, 8 );	//2012/02/23　４→7に。 
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_BLACK, SCR_REBIRTH_TYPE_PENALTY, 8 );	//2012/02/23　４→7に。 
...
function mis_SetRebirthTimeAtCastleEvent()
{
	...
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_BLACK, SCR_REBIRTH_TYPE_NORMAL, 14 );
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_BLACK, SCR_REBIRTH_TYPE_PENALTY, 14 );
	...
	scr_SetRebirthTime( SCR_REBIRTH_TARGET_BLACK, SCR_REBIRTH_TYPE_SPECIAL, 18 );
}
