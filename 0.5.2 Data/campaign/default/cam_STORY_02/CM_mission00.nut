...
//	scr_BotPlayerRegistAndCreate( 104, "NOOB02", POWER_COLOR_WHITE );
	...
	scr_BotPlayerRegistAndCreate( 225, "IQ9_BOSS", POWER_COLOR_BLACK );	//boss 
        ...
        //コールバックの設定をしておくよ！ 
	// Set the callback!
	scr_SetMessageCallback( "miscb_PlayerSally_First", 1, CB_TYPE_ONCE, MESSAGE_PLAYER_SALLY );	//初回出撃した時のコールバック // Callback for the first sortie
	scr_SetMessageCallback( "miscb_TimeOver", 2, CB_TYPE_ONCE,  MESSAGE_GAME_TIME_OVER );		//タイムオーバー時 // When the time is over
	scr_AddDeathEnemyCallback( "miscb_BotDead225", 225 );	//標的発見前撃破 // Defeat before target discovery
	scr_SetTimerCallback( "miscb_BossDamage", 3, CB_TYPE_ONCE, 10.0 );	//ボスのダメージ監視 // Boss damage monitoring
//	scr_SetTimerCallback( "miscb_BossSelfDeath", 4, CB_TYPE_ONCE, 8.3 );	
	scr_AddStandbyEnemyCallback( "miscb_RespawnBoss", 225, STANDBY_COUNT_START_TIMING );	//ボスの自殺監視 // Boss suicide monitoring
